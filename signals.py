...
# -----------------------------------------------------------------------------
# Single-timeframe signal (used by the multi-asset Dashboard)
# -----------------------------------------------------------------------------
def evaluate_single_tf(
    score: float,
    forecasts_dict: Dict[str, str],
    last_price: float,
    cfg: "SignalConfig" = None,
) -> Dict[str, Any]:
    """
    Apply the same BUY/SELL/WAITBUY/WAITSELL/NOACTION logic as `evaluate()`
    but on a single timeframe.  Used by the multi-asset Dashboard to give
    every instrument a trade signal (not just a macro label).

    Rules (matching the gold signal engine):
      BUY     — score >  0  AND all 3 forecasts are Bullish
      SELL    — score <  0  AND all 3 forecasts are Bearish
      WAITBUY — score >  0  AND not all forecasts Bullish
      WAITSELL— score <  0  AND not all forecasts Bearish
      NOACTION— score ≈ 0  (within ±5)
    """
    if cfg is None:
        cfg = SignalConfig()

    bull = sum(1 for v in forecasts_dict.values() if v == "Bullish")
    bear = sum(1 for v in forecasts_dict.values() if v == "Bearish")
    n_forecasts = max(len(forecasts_dict), 1)
    bull_pct = bull / n_forecasts
    bear_pct = bear / n_forecasts

    # Threshold: any value within ±5 is considered NOACTION (matches gold's
    # `min_composite_buy=0` default with a small dead zone to avoid noise)
    dead_zone = 5.0
    if abs(score) < dead_zone:
        signal = "NOACTION"
        reason = f"score {score:+.1f} within ±{dead_zone} dead zone"
    elif score > 0:
        if bull_pct >= 0.99:                       # all 3 Bullish
            signal = "BUY"
            reason = f"score {score:+.1f} + {bull}/3 forecasts Bullish"
        else:
            signal = "WAITBUY"
            reason = f"score {score:+.1f} but only {bull}/3 forecasts Bullish"
    else:                                          # score < 0
        if bear_pct >= 0.99:                       # all 3 Bearish
            signal = "SELL"
            reason = f"score {score:+.1f} + {bear}/3 forecasts Bearish"
        else:
            signal = "WAITSELL"
            reason = f"score {score:+.1f} but only {bear}/3 forecasts Bearish"

    # Probability: same formula as gold's `probability()` — 0.50 + |score|/100*0.45
    prob = 0.50 + min(abs(score), 100.0) / 100.0 * 0.45
    # If a forecast is missing, lower the probability slightly
    if bull + bear < n_forecasts:
        prob = max(0.50, prob * 0.85)

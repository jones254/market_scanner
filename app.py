...
with tab_dashboard:
    ...
    if not all_data:
        ...
        if not st.session_state["dashboard_scores"] or st.button("Rescore", key="dash_rescore"):
            from scoring import composite_score, neg_corr_for
            scores = {}
            ...
            st.markdown(
                f"""<div style="border:2px solid {color};border-radius:8px;
                      ...
                      <div style="font-size:0.75rem;color:#444;margin-top:4px">
                        Score {score:+.1f} · Conf {conf:.0f}% · p={prob*100:.0f}%
                      </div>
        ...
        for asset_class in selected_classes:
            ...
            for row_start in range(0, len(instruments_in_class), cols_per_row):
                ...
                for col, mset in zip(cols, row):
                    with col:
                        _card(mset.symbol, scores.get(mset.symbol))
        ...
        scored = [(sym, sc) for sym, sc in scores.items() if sc is not None]
        if scored:
            n_bull = sum(1 for _, sc in scored if str(sc.label.iloc[-1]) in ("Bullish", "Strong Bullish"))
            n_bear = sum(1 for _, sc in scored if str(sc.label.iloc[-1]) in ("Bearish", "Strong Bearish"))
            n_neut = sum(1 for _, sc in scored if str(sc.label.iloc[-1]) == "Neutral")
            bc1, bc2, bc3, bc4 = st.columns(4)
            bc1.metric("Instruments", len(scored))
            bc2.metric("Bullish",     n_bull, delta=f"{n_bull*100/len(scored):.0f}% of universe")
            bc3.metric("Bearish",     n_bear, delta=f"{n_bear*100/len(scored):.0f}% of universe", delta_color="inverse")
            bc4.metric("Neutral",     n_neut, delta=f"{n_neut*100/len(scored):.0f}% of universe")

            # Top 5 strongest / weakest
            df = pd.DataFrame([
                {"Symbol": sym, "Score": float(sc.composite.iloc[-1]),
                 "Label":  str(sc.label.iloc[-1]),
                 "Conf":   float(sc.confidence.iloc[-1])}
                for sym, sc in scored
            ]).sort_values("Score", ascending=False)
            st.markdown("**Top 5 strongest**")
            st.dataframe(df.head(5), use_container_width=True, height=200)
            st.markdown("**Top 5 weakest**")
            st.dataframe(df.tail(5).iloc[::-1], use_container_width=True, height=200)
        else:
